<?php
class ControllerModuleMobileActivator extends Controller {
	protected function index($setting) {
		...
		if (isset($this->request->get['view_type'])){
			$view_type = $this->request->get['view_type'];
		} elseif (isset($this->session->data['view_type'])){
			$view_type = $this->session->data['view_type'];	
		} elseif (isset($this->request->cookie['view_type'])){
			$view_type = $this->request->cookie['view_type'];
		} else {
			$view_type = 'mobile';
		}
		
		if (!isset($this->session->data['view_type']) || $this->session->data['view_type'] != $view_type) {
			$this->session->data['view_type'] = $view_type;
		}

		if (!isset($this->request->cookie['view_type']) || $this->request->cookie['view_type'] != $view_type) {	  
			setcookie('view_type', $view_type, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
		}
		...
		$this->session->data['mobile_browser'] = ($mobile_browser == true)? 1 : 0;
		
		if ($mobile_browser == true && $view_type == 'mobile' && substr($current_route,0,12) != "mobile_store" && substr($current_route,0,13) != "android_store"){
