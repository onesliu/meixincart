...
class ControllerMobileStoreShipping extends Controller {
	public function index() {
		if (!isset($this->session->data['address_id']) && $this->cart->hasShipping() && $this->config->get('shipping_estimator')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mobile_store/shipping.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/mobile_store/shipping.tpl';
			} else {
				$this->template = 'default/template/mobile_store/shipping.tpl';
			}
