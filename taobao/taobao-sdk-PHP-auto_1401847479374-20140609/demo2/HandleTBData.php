...
class HandleTBData
{
     ...
     */
    public function getTrades($json_str)
    {
     ...
     */
    public function getIncrementTrades($json_str)
    {
        $arr = json_decode($json_str);
     ...
     */
    public function getTradeFullInfo($json_str)
    {
        $arr = json_decode($json_str);
     ...
     */
    public function updateItemPrice($json_str)
    {
        $arr = json_decode($json_str);
        ...
        foreach($this->resp->trades->trade as $paraValue)
        {
            switch ($paraValue->status)
            {
                ...
                case "TRADE_FINISHED":              //(交易成功)
                {
                    ...
                    {
                        $this->getTradeFullInfo(json_encode(array('fields' => "buyer_message",
                                                                  'tid' => $paraValue->sid)));
                    ...
                    {
                        $query = $db->query("SELECT * FROM oc_product_description WHERE name = '".$arr->title."'");
                        if ( $query->num_rows == 0 )
                        {
